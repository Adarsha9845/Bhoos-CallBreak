...
class Player:
    ...
    def playCard(self, playContext,train,lossFn,delta):
        ...
        # print("\n")
        #######
        # or also playContext["cards"] this is the list of the cards with the player
        actions = self.possibleCards(playContext["played"])
        inputs = []  # jpt value, it is the input to the newral network, made using playCOntext and
        
        qvalues = []  # empty list, this is generated from the self.network.feedForward

        for action in actions:
            inputs.append(self.input_encoding(playContext,action))  # this must be the combination of the derived inputs and the action
        z,a=self.network.feedForward(np.array(inputs))
        if len(a[-1])==1:
            qvalues=[a[-1].squeeze().tolist()]
        else:
            qvalues=a[-1].squeeze().tolist()
        option = np.random.random()
        if option <= self.epsilon:  # exploration
            action = np.random.randint(len(qvalues))
        else:  # exploitation
            action = qvalues.index(max(qvalues))
        hand = actions[action]
        self.historyOfQ.append(qvalues[action])#for plotting purposes
        self.currentQ = qvalues[action]
        ########insert code here##############
        selectedInput=inputs[action] #this is the input generated from the present state and actions[action]
        ######################################
        z,a=self.network.feedForward(np.array([selectedInput])) #these are the activations and outputs
        #######
    ...
    def cardEncoding(self, cards):
        #redesign this as the feature seleciton is not suitable
        my_card = [0] * 26
        counter = 0
        for card in cards:
            my_card[counter] = Cards.getRank(card)['value']
            counter = counter + 1
            my_card[counter] = Cards.getSuit(card)['value']
            counter = counter + 1
        return my_card   
    ...
    def input_encoding(self, playInfo, possible_move):
        # remaining card in the hand..
        my_card = self.cardEncoding(playInfo['cards'])
        ...
        # finalInputEncoding = played_card+largestSmallestNum+possibleMove
